...
def index_user(user): #cria doc para usuarios
    doc = {
        "id": f"user_{user.id}",
        "user_id_i": user.id,
        "name_t": user.name,
...
def index_habit(habit): #cria doc para habitos
    doc = {
    ...
    requests.post( #envia para o solr
...
def delete_document(core, doc_id): #remove o doc do index
    requests.post(
        f"{SOLR_URL}/{core}/update",
        json={"delete": {"id": doc_id}}
    )
    solr_commit(core) #confirma deleçao
