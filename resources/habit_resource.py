...
class HabitResource(Resource):
    ...
    def get(self, habit_id=None):
        ...
        if habit_id is not None:
            ...
            # sequencia sempre representa "hoje"
            today_count = (
        ...
        # mapa do habit_id para contagem de conclusões de hoje
        today_map = {}
        if habit_ids:
            rows = (
    ...
    def post(self):
        ...
        difficulty = data.get("difficulty") or "facil" #valores padrões
        ...
        db.session.commit()
        index_habit(habit) #indexar o habito no solr
    ...
    def put(self, habit_id):
        ...
        habit = Habit.query.filter_by(id=habit_id, user_id=user_id).first()
        if not habit:
            return {"message": "Hábito não encontrado"}, 404 #atualiza os campos que vem na requesisão
    ...
    def delete(self, habit_id):
        ...
        habit_solr_id = f"habit_{habit.id}" #ID no solr

        db.session.delete(habit)
        db.session.commit()

        delete_document("habits", habit_solr_id) # remove do solr
